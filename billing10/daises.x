PROCEDURE DAILY.SESS.UPD.X             % DAISUX.PR2    GD05W0.ME2  10.0

Version 10.0 - November 19, 1986 - Mark Nichols
  Added another line of instructions to be typed out for the benefit of the
operators at the end of the procedure that asks them to copy the storage and
session file for the day to directory STLOUOPS.

Version 9.0 - November 17, 1986 - Mark Nichols
  Modified format of header and checksum records on SESS.FIL to include
trailing blanks in preparation for migration of service billign to St. Louis.
...
SOURCE FILE.CTL FROM 'DAILY.CTL' FIXED FORM
  INPUT REC.TYPE,
...
RELATION DUP.CHECK IS
  KEY   DSTART, TSTART, UUN
  DATA  JOB

SOURCE CNTRL.FIL FROM 'SESION.CTL' FREE FORM
  INPUT FIRST.SESS,
	LAST.SESS
...
FORMAT RUN.DATE.TIME
  @TAB 3, TODAY AS DATE 'MM/DD/YY', @TAB 2,
  (TIME/3600) AS '2Z', ":", ((TIME-TIME/3600*3600)/60) AS '2N', @CR

SOURCE DAILYX.DATA FROM IN.FNAM FIXED FORM LINE.LENGTH 290
  INPUT REC.TYPE,
	JOB,
	TID,
	GAN,
	...
	GROUP.NUM, @CR
      2 OR 3:
	DSTOP.X, % 2 - Chkpnt record - equates type 22 %
...
TYPE @CR, "Beginning DAILY SESSION UPDATE.....Version 10.0", RUN.DATE.TIME
...
FOR EACH FILE.CTL
  CONDITIONAL ON REC.TYPE
    BEGIN % 3 - %
    1:BEGIN % 4 - %
      IF HOST = 0 AND BILLING.HOST = 0 
	THEN ABORT @CR,'PRIMARY ACTG HAS NOT BEEN RUN',@CR
	ELSE NOTHING
      MOVE HOST TO HOST.L
...
SELECT SRC.FNAMS VIA KEY 
  THEN ABORT @CR,'FILE:  ',IN.FNAM,'  ALREADY PROCESSED',@CR
  ELSE NOTHING
...
WRITE REPORTS SESS.FIL, ERR.FIL
  ...
  PRINT TO SESS.FIL
      TRAN.TYP, FIL.DATE.L, HOST.L, 169 * ' ', @CR
  MOVE 1 TO TRAN.TYP
  FOR EACH RAW.DATA     % READ RAW.DATA AND MATCH RECORDS %
	...
	RPTSEL:
	  ...
	  SELECT X.SUSPENSE VIA KEY 
	    THEN % DO WE HAVE A PREVIOUS RECORD IN SUS %
	      BEGIN % 5 - %
	      IF UUN OF X.SUSPENSE = UUN OF RAW.DATA 
		THEN % IF SO ARE UUNS SAME %
		  BEGIN % 6 - %
		  LET UUNTST = 'EQ'
		  IF REC.TYPE # 1 
		    THEN % IGNORE LOGIN - NO LOGOUT FOR SESSION IN SUSPENSE  %
		      IF CNC.HRS.X LE CNC.HRS.X OF RAW.DATA 
			THEN
			  BEGIN % 8 - % % YES - UPDATE SUSPENSE %
			  IF (REC.TYPE = 4) AND
			     (PROJ.CODE OF X.SUSPENSE = PROJ.CODE OF RAW.DATA) 
			    THEN FINISH RPTSEL
			    ELSE NOTHING
			  ...
			  IF REC.START = 4 AND PC.SESS # 'Y' 
			    THEN %PC.CHANGE RECORD%
			      BEGIN % 9 - %
			    ...
			    ELSE NOTHING
			  IF REC.TYPE # 4 
			    THEN % SET NODE, PORT IF 0 IN SUS %
			      IF NODE OF X.SUSPENSE # 0 
				THEN NOTHING
				ELSE
				  BEGIN % 10 - %
			  ...
			  MOVE ENTERS.RENAMES.SIZE OF RAW.DATA TO 
			       ENTERS.RENAMES.SIZE
			  MOVE DISC.BLKS.IN.SIZE OF RAW.DATA TO 
			       DISC.BLKS.IN.SIZE
			  MOVE DISC.BLKS.OUT.SIZE OF RAW.DATA TO 
			       DISC.BLKS.OUT.SIZE
			  ...
			  MOVE MICRO.CYCLES.SIZE OF RAW.DATA TO 
			       MICRO.CYCLES.SIZE
		    ...
		    ELSE
		      ...
		      END % 7 - % % REC.TYPE TEST %
		  END % 6 - %
		ELSE  % UUNS # - CLOSE X.SUSPENSE - PROCESS R.DATA REC %
		  BEGIN % 6 - % 
		  LET UUNTST = 'NE'
		  MOVE 'Y' TO DELSUS
		  MOVE 'N' TO FINSEL
	      ...
	      IF ((REC.START = 1 OR 2 OR 3 OR 4) AND 
	          (REC.STOP = 2 OR 3 OR 4)) OR
		 ((REC.START = 2) AND (REC.STOP = 0)) 
		THEN   
		  DUPCHK:
		    BEGIN % 6 - % % YES - IS IT UNIQUE %
		    SELECT DUP.CHECK VIA KEY 
		      THEN    % IF JOBS = THEN TRUE  DUPLICATE %
			IF JOB OF DUP.CHECK # JOB OF X.SUSPENSE 
			  THEN    % ELSE INCREMENT TIME TILL IT IS UNIQUE %
			    BEGIN % 8 - %
			    IF TSTART OF X.SUSPENSE = TSTOP OF X.SUSPENSE 
			      THEN ADD .0001 TO TSTART OF X.SUSPENSE,
			                        TSTOP OF X.SUSPENSE
			      ELSE ADD .0001 TO TSTART OF X.SUSPENSE
			    REPEAT DUPCHK
			    END  % 8 - %
			  ELSE    % GAD - IT IS A TRUE DUPLICATE - ERROR %
			    BEGIN % 8 - %
			    MOVE 'Y' TO SESS.ERR
			    MOVE 'Y' TO DELSUS
			    ...
			    END % 8 - %      
		      ELSE    % WE HAVE VALID AND UNIQUE SESSION RECORD %
			BEGIN % 7 - %
			INSERT INTO DUP.CHECK

			%  COMPUTE DETACHED TIME  %

			LET CNC.ELAPSED = ((DSTOP - DSTART) * 24) +
			                  (TSTOP - TSTART)
			LET CNC.DETACHED = (CNC.ELAPSED - CNC.HRS.X)
			IF CNC.DETACHED LT .0083 
			  THEN   
			    BEGIN % 8 - %
			...
			IF REC.START = 1 
			  THEN % INCREMENT CATEGORY COUNTER %   
			...
			IF REC.START = 4 
			  ...
			  ELSE NOTHING   
			IF REC.START = 3 
			  THEN ADD 1 TO LOGOUT.ONLY
			  ELSE NOTHING   
			IF REC.STOP = 4 
			  THEN    % PC change - do not charge for SESS %
			    BEGIN % 9 - %
			    MOVE 'N' TO SESS.CHG
			...
			IF TRUS < 0 
			  ...
			  ELSE NOTHING
			PRINT TO SESS.FIL
			  TRAN.TYP, JOB, TID, GAN, UUN, DSTTSTART,
			  CIN, COUT, CNC.HRS.X, NODE, PORT, HOST,
			  CNC.SEC.DISPATCH, CIO.REMOTE, XCHARG,
			  CNC.SEC.BLOCK.IO, CIO.BLOCK.IO, TRUS, DSTOP, TSTOP,
		...
		ELSE
		  ...
		  IF REC.START = 1 
		    THEN ADD 1 TO LOGIN.ONLY
		    ELSE ADD 1 TO PC.CHANGE.ONLY
		  END % 6 - % % REC.TYPE TEST %
	      IF SESS.ERR = 'Y' 
		THEN PRINT TO ERR.FIL
		...
		ELSE NOTHING
	      IF PCUUN = 'Y' 
		THEN  % WAS LAST REC.TYPE 4 - IF SO SET UP DUMMY
		...
		ELSE NOTHING
	      IF DELSUS = 'Y' 
		THEN DELETE FROM X.SUSPENSE VIA KEY
		ELSE NOTHING
	      IF UUNTST = 'EQ'   % IF UUNS = THEN DONE WITH CURRENT RECORD %
		THEN MOVE 'Y' TO FINSEL   % ELSE WE STILL HAVE TO PROCESS IT %
		ELSE MOVE 'N' TO FINSEL
	      IF FINSEL = 'Y' 
		THEN FINISH RPTSEL
		ELSE REPEAT RPTSEL
	      END % 5 - % % SCOPE OF SELECT ON X.SUSPENSE %
	    ELSE  % NO PREVIOUS RECORD IN X.SUSPENSE FOR THIS SESSION %
	      BEGIN % 5 - %
	      MOVE REC.TYPE TO REC.START
		...
		2:BEGIN % 7 - %
		  ...
		  WHILE TSTART LT 0 DO
		    BEGIN % 8 - % % COMPUTE LOGON %
	...
	TYPE @CR,'Encountered UP-TIME RECORD dated  ',DSTART AS DATE
             'ZM/ZD/YY',@TAB 2,TSTART, @CR, @CR,
	     COUNT(X.SUSPENSE), " RECORD(S) being CLEARED from SUSPENSE", @CR
        FOR EACH X.SUSPENSE
	  BEGIN % 5 - %
  ...
  PRINT TO SESS.FIL
    TRAN.TYP, UUN.SUM, FIRST.SESS, LAST.SESS, DAILY.DATE, 146 * ' ', @CR
  END % 2 - % % WRITE REPORTS SESS.FIL, ERR.FIL %

SELECT MONTHLY.STATS VIA KEY 
  THEN % UPDATE STATISTICS RELATION %
    BEGIN % 2 - %
    ...
    ALTER MONTHLY.STATS
    END % 2 - %   
  ELSE INSERT INTO MONTHLY.STATS
...
FOR EACH X.SUSPENSE
  DUPCHK2:
	...
	MOVE 'Y' TO DUP.FLAG
	IF TSTART OF X.SUSPENSE = TSTOP OF X.SUSPENSE 
	  THEN ADD .0001 TO TSTART OF X.SUSPENSE, TSTOP OF X.SUSPENSE
	  ELSE ADD .0001 TO TSTART OF X.SUSPENSE
	REPEAT DUPCHK2
	END % 3 - %
      ELSE
...
IF SESS.CT # CHKPNT.CHKPNT + CHKPNT.LOGOUT + CHKPNT.ONLY + CHKPNT.PC.CHANGE +
             ...
             PC.CHANGE.PC.CHANGE 
  THEN TYPE @CR,@CR,
            'WARNING=====> SESSIONS ON FILE NOT EQUAL TO ABOVE SUMMARY',@CR
  ELSE NOTHING

IF HOST.L NE BILLING.HOST.L 
  THEN TYPE @CR, "Please move the files ", OUT.FNAM AS '10C', " and ",
        STG.FNAM, @CR, @CR, "to the Central Accounting System - Host",
        BILLING.HOST.L, " (After ROYALTY is run)", @CR
  ELSE NOTHING

TYPE @CR, "Please copy the files ", OUT.FNAM AS '10C', " and ", STG.FNAM, @CR,
     "to this host's STLOUOPS directory after ROYALTY is run.", @CR
